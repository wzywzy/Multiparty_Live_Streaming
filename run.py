...
import time as tm
#cooked data
rf = open('rebuffer_single.txt', 'w') 
#writerQoe = writer(mf)
qf = open('Qoe_LP+BFA','w')
bf = open('bitrate_single.txt','w')
jf = open('jitter_single.txt','w')
...
for file in files:
    with open(FILE_PATH+file, 'r') as f:
        ...
        for line in f:
            if line[0]!='p':
                bandwidth.extend([int(line)/30]*15)
...
for count in range(len(AvgThpDown)):
    ...
    uplink_trace[count]  =np.array([500]*20000)
...
init_estimate_1 = []
for i in range(usercount):
    trace_to_estimate = uplink_trace[i][0:5].tolist()
...
for time in range(20000):
    ...
    if(time%100 == 1):
        ...
        for reciever in Recievers:
            ...
            jitter = average_bitrate - average_bitrate_last[reciever.id]
            #print(jitter)
            #jf.write(str(jitter)+'\n')
    ...
    if(time%LP_GAP==LP_GAP-1):
        estimate = []
        for i in range(usercount):
            trace_to_estimate = downlink_trace[i][time-5:time]
            estimate.append(algrithm.estimate(trace_to_estimate))
        estimate_1 = []
        for i in range(usercount):
            trace_to_estimate = uplink_trace[i][time:time+5].tolist()
    ...
    if(time%FB_GAP==1):
        ...
        estimate = []
        for i in range(usercount):
            trace_to_estimate = downlink_trace[i][time:time+5]
            estimate.append(algrithm.estimate(trace_to_estimate))
        change = []
        change.clear()
        for i in range(usercount):
        ...
        time_start = tm.time_ns()
        result,error_sum,lastbuffer = algrithm.bitratecontrol(Recievers,Senders,result,estimate,lastbuffer,lastrebuffer,FB_GAP,usercount,error_sum,change)
        time_end = tm.time_ns()
        time_last = time_end-time_start
